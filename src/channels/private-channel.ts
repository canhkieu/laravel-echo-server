...
export class PrivateChannel {
  ...
  protected hasMatchingHost(referer: any, host: any): boolean {
    return (
      referer.hostname.substr(referer.hostname.indexOf(".")) === host ||
      ...
      this.request.post(options, (error, response, body, next) => {
        if (error) {
          if (this.options.devMode) {
            Log.error(
              `[${new Date().toLocaleTimeString()}] - Error authenticating ${
                socket.id
          ...
          reject({
            reason: "Error sending authentication request.",
            status: 0
          });
          ...
          if (this.options.devMode) {
            Log.warning(
              `[${new Date().toLocaleTimeString()}] - ${
                socket.id
              } could not be authenticated to ${options.form.channel_name}`
            );
          ...
          reject({
            reason:
              "Client can not be authenticated, got HTTP status " +
              response.statusCode,
          ...
          if (this.options.devMode) {
            Log.info(
              `[${new Date().toLocaleTimeString()}] - ${
                socket.id
              } authenticated for: ${options.form.channel_name}`
            );
          }
